...
class MenuWidget extends Controller{
	...
	function menu_show(){
		...
		$menu=M('seller_menu')->where('is_mall < 2')->order('sort_order')->select();
		...
		if(!$_SESSION[C('ADMIN_AUTH_KEY')] && false)
		{
			...
			$menu=M('seller_menu')->where( 'id in ('.$child_menu_str.')' )->order('sort_order')->select();
	...
	private function _get_parent_menu($menu,&$child_menu)
	{
		if($menu['pid'] !=0)
		{
			$tmp_menu = M('seller_menu')->where( array('id' => $menu['pid']) )->find();
